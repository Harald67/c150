...
// this is taken from default.frag
    ...
    {
        // n is in world space
        vec4 nmap       = texture2D(normal_texture, gl_TexCoord[0].st * nmap_tile);
        vec3 N = nmap.rgb * 2.0 - 1.0;
        N = normalize(N.x * VTangent + N.y * VBinormal + N.z * n);
        if (nmap_dds > 0)
            N = -N;
        n = N;
    }
...
//fragColor.rgb = nmap.rgb;
//fragColor.rgb = VBinormal.rgb;
